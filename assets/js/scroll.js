...
//variable para las páginas en las que cambia el logo
  ...
  $(logo).attr('src', 'https://zerty.cl/wp-content/themes/zerty/assets/img/zerty_blanco.png');
...
$(window).scroll(function () {
 if($(this).scrollTop() > 300){
     if (!$('#site-navigation').hasClass('fixed-top')){
         ...
         $(logo).attr('src', 'https://zerty.cl/wp-content/themes/zerty/assets/img/zerty_normal.png');
     }
 }else{
 $('.navbar').removeClass('fixed-top navbar-light bg-white').addClass('navbar-dark').css({'-webkit-box-shadow': '', 'box-shadow': '', 'background-color': '#4448d3'});
 $('.home #site-navigation').removeClass('navbar-dark').addClass('navbar-light').css({'background-color': ''});
 $(logo).attr('src', 'https://zerty.cl/wp-content/themes/zerty/assets/img/zerty_blanco.png');
 }
});
